...
RUN apk update && apk add -u python=$PYTHON_VERSION py-pip=$PY_PIP_VERSION && pip install supervisor==$SUPERVISOR_VERSION
RUN echo "date.timezone = UTC" >> /usr/local/etc/php/conf.d/opencats.ini && \
     echo "realpath_cache_size = 4096k" >> /usr/local/etc/php/conf.d/opencats.ini && \
